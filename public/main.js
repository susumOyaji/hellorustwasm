...
async function init() {
    try {
        ...
        try {
            wasm = await import('./pkg/hellorustwasm.js');
            await wasm.default();
...
async function fetchSonyStock() {
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            // Display stock information using DOW portfolio style
            const stockInfo = `
    ...
    try {
        ...
        if (result.success && result.data && result.data.length > 0) {
            ...
            result.data.forEach(stock => {
                const shares = stock.holdings.shares;
                const purchasePrice = stock.holdings.purchase_price;
                const currentPrice = (wasm && wasm.parse_stock_price) ? wasm.parse_stock_price(stock.current_price) : parseFloat(stock.current_price.replace(/[^\d.-]/g, '')) || 0;
